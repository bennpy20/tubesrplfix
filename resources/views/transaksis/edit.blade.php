<form action="{{ route('transaksis.update', $transaksi->id) }}" method="POST" class="p-4 md:p-5" autocomplete="off">
    @csrf
    @method('PUT')
    <div class="grid gap-4 mb-4 grid-cols-2">
        <div class="col-span-2">
            ...
            form-control @error('label') is-invalid @enderror" value="{{ old('label', $transaksi->label) }}" placeholder="Masukkan nama transaksi" required />
        ...
        <div class="col-span-2 sm:col-span-1">
            ...
            form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $transaksi->amount) }}" placeholder="Masukkan harga" oninput="this.value = Math.max(0, this.value)" step="any" required />
        ...
        <div class="col-span-2 sm:col-span-1">
            ...
            form-control @error('jenis') is-invalid @enderror" value="{{ old('jenis', $transaksi->jenis) }}">
        ...
        <div class="col-span-2">
            ...
            <div class="relative max-w-sm form-control">
                ...
                @error('date') is-invalid @enderror" value="{{ old('date', $transaksi->date) }}" placeholder="Pilih tanggal">
            </div>
        ...
        <div class="col-span-2">
            ...
            form-control @error('note') is-invalid @enderror" placeholder="Tulis keterangan...">{{ old('note', $transaksi->note) }}</textarea>                    
        </div>
        ...
        <div class="col-span-2">
            ...
            form-control @error('id_dompet') is-invalid @enderror" value="{{ old('id_dompet') }}">
                @foreach ($dompets as $dompet)
                    <option value="{{ $dompet->id }}" {{ old('id_dompet', $transaksi->id_dompet ?? '') == $dompet->id ? 'selected' : '' }}>
                        {{ $dompet->label }}
                    </option>
                @endforeach
            </select>
            <!-- error message untuk id_dompet -->
            @error('id_dompet')
                <div class="alert alert-danger mt-2">
                    {{ $message }}
                </div>
    ...
    <div class="flex justify-end">
        <button type="submit" class="text-white bg-yellow-400 hover:bg-yellow-500 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700">Simpan</button>
    </div>
</form>